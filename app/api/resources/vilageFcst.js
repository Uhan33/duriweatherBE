...
router.get("/ptycheck", async (req, res) => {    // 없음 : 0, 비 : 1, 비/눈 : 2, 눈 : 3, 소나기 : 4
    ...
    for(var i = 0; i < 1000; i++) {
        if(hour == data.response.body.items.item[i].fcstTime && now == data.response.body.items.item[i].fcstDate && "PTY" == data.response.body.items.item[i].category)
            break;
    }
    res.send(data.response.body.items.item[i]);
});
...
router.get("/reh", async (req, res) => {    // %로 내보내는듯.
    ...
    for(var i = 0; i < 1000; i++) {
        if(hour == data.response.body.items.item[i].fcstTime && fNow == data.response.body.items.item[i].fcstDate && "REH" == data.response.body.items.item[i].category)
            break;
    }
    res.send(data.response.body.items.item[i].fcstValue);
});
...
router.get("/wsd", async (req, res) => {  
    ...
    for(var i = 0; i < 1000; i++) {
        if(hour == data.response.body.items.item[i].fcstTime && fNow == data.response.body.items.item[i].fcstDate && "WSD" == data.response.body.items.item[i].category)
            break;
    }
    res.send(data.response.body.items.item[i].fcstValue);
});
...
router.get("/vec", async (req, res) => {  
    ...
    for(var i = 0; i < 1000; i++) {
        if(hour == data.response.body.items.item[i].fcstTime && fNow == data.response.body.items.item[i].fcstDate && "VEC" == data.response.body.items.item[i].category)
            break;
    }
    res.send(data.response.body.items.item[i].fcstValue);
});
...
router.get("/pop", async (req, res) => {  
    ...
    for(var i = 0; i < 1000; i++) {
        if(hour == data.response.body.items.item[i].fcstTime && fNow == data.response.body.items.item[i].fcstDate && "POP" == data.response.body.items.item[i].category)
            break;
    }
    res.send(data.response.body.items.item[i].fcstValue);
});
...
router.get("/tmn", async (req, res) => {  
    ...
    for(var i = 0; i < 1000; i++) {
        if(fNow == data.response.body.items.item[i].fcstDate && "TMP" == data.response.body.items.item[i].category && "0600" == data.response.body.items.item[i].fcstTime)
            break;
    }
    res.send(data.response.body.items.item[i].fcstValue);
});
